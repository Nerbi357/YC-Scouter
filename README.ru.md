...
**Как это выглядит на практике:** сузить список до финтеха последних двух батчей,
который ещё нанимает, открыть карточку, прочитать два риска, которые стоит проверить
до разговора, поставить тег `watch` и перейти к следующей компании. **Датасет лежит
в репозитории**, поэтому `streamlit run app.py` работает сразу после клонирования —
ключ нужен только чтобы пересобрать данные, но не чтобы их смотреть.
...
- **Искать** — фильтры по индустрии и подиндустрии, статусу YC, investability, году
  батча, размеру команды, custom score, вашей стадии воронки и тегам; поиск по названиям,
...
streamlit run app.py                                # дашборд на закоммиченных данных
pytest                                              # тесты
```

Пересборка данных — отдельный необязательный шаг: пайплайн запускается через тетради
(Colab или `papermill`) либо через две кнопки в GitHub Actions. Ключ
`ANTHROPIC_API_KEY` (или `GROQ_API_KEY`) нужен только этому шагу; без ключа AI-колонки
заполняются заглушкой и ничего не списывается.
...
- **File 1** — `notebooks/01_dataset_base.ipynb`: забирает все компании YC из
  ...
  нормализует, добавляет `investability`, ссылки для изучения и `custom_score` 0–100, пишет
...
- [`HOW_IT_WORKS.md`](DOCS/HOW_IT_WORKS.md) — архитектура, производные поля, промпты
  и контроль стоимости, контракт воспроизводимости.
- [`HOW_TO_DEPLOY_DASHBOARD.md`](DOCS/HOW_TO_DEPLOY_DASHBOARD.md) — пошаговый деплой
  с нуля.
- [`HOW_TO_UPDATE.md`](DOCS/HOW_TO_UPDATE.md) — обслуживание: две кнопки, ключи,
  модели, зависимости, что ломается и что с этим делать.
- Продолжаете проект с ИИ-агентом? Начните с
  [`AI_USAGE/PROJECT_MEMORY.md`](AI_USAGE/PROJECT_MEMORY.md) — решения и их причины,
  текущее состояние, открытые задачи; рядом
  [`AI_INSTRUCTIONS.md`](AI_USAGE/AI_INSTRUCTIONS.md) и
  [`IDEAS.md`](AI_USAGE/IDEAS.md).

## Как читать цифры

Часть полей приходит от YC, часть посчитана этим проектом — поэтому единственное
число, которое мы придумали сами, называется `custom_score`, а не «score». Карточка
их не смешивает — и эта таблица тоже.
...
| `custom_score` (0–100) | **этот проект** | Прозрачная взвешенная сумма дешёвых сигналов: флаг *top company* от YC (вес 3), свежесть батча (2), открытые вакансии (1), размер команды (1), содержательное описание (0.5) и богатство тегов (0.5). Детерминирован и перенастраивается в [`src/yc_scouter/score.py`](src/yc_scouter/score.py). Шкала намеренно строгая: в прогоне 2026-07-25 медиана — 23.4, максимум — 72.9. |
...
- **Только открытые источники.** Данные — из зеркала сообщества `yc-oss/api` (без
  ключей, пересобирается ежедневно); все ссылки ведут на свободно доступные страницы.
- **Ничего не выдумывается.** Раунды, оценки и cap table частных стартапов нигде не
  публикуются, поэтому их нет и здесь: неизвестный факт остаётся пустым, а карточка
  предлагает ссылку. AI-поля написаны **только** по опубликованному тексту самой
  компании.
- **Воспроизводим код, а не данные.** Окружение зафиксировано хешированным
  lock-файлом; источник между прогонами законно меняется, поэтому каждый прогон
  пишется в свою датированную пару файлов, а прежние остаются нетронутыми.
