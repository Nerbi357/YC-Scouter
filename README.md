...
**Statement.** Conceptualisation, requirements, all product decisions and final
review: the author. Implementation, test suites and documentation drafts: an AI
coding agent working under the author's direction and reviewed by them. Two fields
of the dataset (`ai_description`, `ai_risks`) are generated by a large language
model from public Y Combinator text and are labelled as machine-generated wherever
they appear; the exact model, prompts and cost controls are documented in
[`DOCS/HOW_IT_WORKS.md`](DOCS/HOW_IT_WORKS.md#5-the-ai-step--prompts-parameters-cost).
No figures that the source does not publish are produced by the model. The author
takes full responsibility for the content of this repository.

The working method itself — how the agent was directed, verified and constrained —
is documented in [`AI_USAGE/`](AI_USAGE/AI_METHODOLOGY.md) so the same process can
