...
- Репозиторий: **`Nerbi357/VC-Tracking`** (переименование в `YC-Scouter` обсуждалось,
  но **ещё не сделано** — см. «Что дальше»; проект внутри везде называется YC Scouter).
...
3. **Репетиция рутины владельца**: нажать обе кнопки Actions и убедиться, что новый
   датированный датасет подхватывается дашбордом. Стоимость почти нулевая — кэш
   AI платит только за компании, появившиеся после 2026-07-24.
4. **Переименование репозитория** `VC-Tracking` → `YC-Scouter` (решение владельца).
   Важно: после переименования переподключить репозиторий в настройках Streamlit
   Cloud, иначе деплой отвалится.
5. **File 5 из исходной спеки** («тетрадь, готовящая деплой дашборда») отдельной
   тетрадью не делался — его роль выполняют `app.py` + `docs/DEPLOY.md`. Либо
   считаем закрытым, либо делаем тонкую тетрадь-чеклист деплоя.
6. Возможно: фаза 2 — полноценный сайт (Cloudflare Pages поверх того же датасета).
